...
<%
	...
	long organizationLogoId = (long) request.getAttribute("organizationLogoId");

	Date createDate = currentUser.getCreateDate();
	String currentYear = createDate.toLocaleString().substring(7, 12);
%>

<div class="profile_info row">
	<div class="d-flex align-items-center col-md-10">
		<img class="rounded-circle" src="<%=profilePicture%>" alt="">
		<div class="flex-column ml-3">
			<h5>
				Welcome
				<%=currentUser.getFirstName()%>&nbsp;<%=currentUser.getLastName()%></h5>
			<div>
				<a href=""><svg class="svg-inline--fa fa-envelope fa-w-16"
					...
					<%=currentUser.getEmailAddress()%></a> <a class="pl-2" href=""><svg
					...
					<!-- <i class="fas fa-phone"></i> Font Awesome fontawesome.com -->
					1800 500 000</a>
			</div>
			<div>
				User #<%=currentUser.getUserId()%>
				<span class="pl-5">Member since <%=currentYear%></span>
			</div>
		</div>
	</div>
	<div class="col-md-2">
		<img style="width: 100px; height: 100px;" class="rounded-circle"
			src="/image/organization_logo?img_id=<%=organizationLogoId%>&amp;">
	</div>
</div>

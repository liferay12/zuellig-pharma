...
renderResponse.setTitle("Create  Account");
...
<aui:form action="<%= createAccountURL %>" method="post" name="fm" validateOnBlur="<%= false %>">
	...
	<liferay-ui:message arguments="<%= String.valueOf(upe.minLength) %>" key="that-password-is-too-short" translateArguments="<%= false %>" /></liferay-ui:error>
	...
	<liferay-ui:message arguments="<%= HtmlUtil.escape(upe.regex) %>" key="that-password-does-not-comply-with-the-regular-expression" translateArguments="<%= false %>" /></liferay-ui:error>
...
<aui:input model="<%= User.class %>"  field="firstName" name="firstName" label="First Name" />
<aui:input model="<%= User.class %>"  field="middleName" name="middleName" label="Middle Name"/>
<aui:input model="<%= User.class %>"  field="lastName" name="lastName" label="Last Name"/>
<%@ taglib uri="http://liferay.com/tld/expando" prefix="liferay-expando" %>

<liferay-expando:custom-attribute className="<%= User.class.getName() %>" classPK="<%= 0 %>" editable="<%= true %>" label="<%= true %>" name="Client Type"/>
			...
			%>
				...
				<%-- <aui:input autoFocus="<%= autoGenerateScreenName  %>" model="<%= User.class %>" name="screenName" label="User Id" disabled="true" >
				</aui:input>--%>
		...
		>
			...
			<c:choose>
				<c:when test="<%= PrefsPropsUtil.getBoolean(company.getCompanyId(), PropsKeys.FIELD_ENABLE_COM_LIFERAY_PORTAL_KERNEL_MODEL_CONTACT_BIRTHDAY) %>">
					<aui:input name="birthday" value="<%= birthdayCalendar %>" label="Date Of Birth"/>
				</c:when>
				<c:otherwise>
...
<div class="d-flex justify-content-center align-item align-items-center">
	<aui:button-row>
		<aui:button type="submit" id="_com_liferay_users_admin_web_portlet_UsersAdminPortlet"/>
	</aui:button-row>
	<div class="signbutton"><%@ include file="/navigation.jspf" %></div>
	</div>
</aui:form>
...
$(document).ready(function(){
	...
	$("[id$='User Type']").val(userType).change();
	$("[id$='User Type']").attr("disabled", true);
});

function getUrlParams(){
	var params = new window.URLSearchParams(window.location.search);
	var queryParam = {};
	params.toString().split("&").forEach(function(item) {queryParam[item.split("=")[0]] = item.split("=")[1]})
	return queryParam;
}
